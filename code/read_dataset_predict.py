            ...
            if src != dst:
                ...
                if G.has_edge(src, dst):
                    D_G.add_node('l_{}_{}'.format(src, dst),
                                ...
                                capacity_no_norm=G.edges[src, dst]['bandwidth'],
                                occupancy=P[src][dst]['qosQueuesStats'][0]['avgPortOccupancy'] /
                                            G.nodes[src]['queueSizes'],
                                queue_size=G.nodes[src]['queueSizes']
                                )
                ...
                for f_id in range(len(T[src, dst]['Flows'])):
                    if T[src, dst]['Flows'][f_id]['AvgBw'] != 0 and T[src, dst]['Flows'][f_id]['PktsGen'] != 0:
                        D_G.add_node('p_{}_{}_{}'.format(src, dst, f_id),
                                     ...
                                     traffic_no_norm=T[src, dst]['Flows'][f_id]['AvgBw']
                                     )
...
def hypergraph_to_input_data(hypergraph):
    ...
    return {"traffic": list(nx.get_node_attributes(D_G, 'traffic').values()),
            "traffic_no_norm": list(nx.get_node_attributes(D_G, 'traffic_no_norm').values()),
...
def input_fn_predict(data_dir, shuffle=False, samples=None):
    ds = tf.data.Dataset.from_generator(generator,
                                        args=[data_dir, shuffle],
                                        output_types=(
                                            {"traffic": tf.float32,
                                             "traffic_no_norm": tf.float32,
                                             "packets": tf.float32,
                                             "capacity": tf.float32,
                                             "queue_size": tf.float32,
                                        ...
                                        output_shapes=(
                                            {"traffic": tf.TensorShape([None]),
                                             "traffic_no_norm": tf.TensorShape([None]),
